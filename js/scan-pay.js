...
(function () {
  ...
  function handleFileUpload(e) {
    ...
    img.onload = () => {
      ...
      if (code && code.data) {
        ...
        window.UIKit.toast("Couldn't detect a QR code in that image. Try a clearer screenshot.", "error");
      }
    };
    img.onerror = () => window.UIKit.toast("Couldn't read that image file.", "error");
  ...
  function handleDecodedText(text) {
    ...
    renderResult(parsed, result, amount);
  }
  ...
  function renderResult(parsed, result, amount) {
    ...
    area.innerHTML = `
      ...
      <div class="app-card" style="padding:14px;margin-bottom:14px;background:#f9fafc;border:1px solid #eef0f6;">
        ...
        <div style="display:flex;justify-content:space-between;font-size:13.5px;"><span>Amount</span><strong>${amount ? fmt(amount) : "Not specified"}</strong></div>
      </div>
